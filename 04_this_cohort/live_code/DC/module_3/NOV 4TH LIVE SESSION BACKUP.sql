...
--or if tou wanted to see how it was counted/confirm the above 
SELECT product_id, COUNT(DISTINCT product_id) as bought_prods
FROM customer_purchases
GROUP BY product_id

-- to see the list of distinct products, display the product_id
...
,strftime('y%-%m-%d','now','+50 days') as [the_future], market_date
,strftime()
STRFTIME ('%y-%m-%d', '2025-11-11', 'start of month')
...
/* 4. difference between dates, 
 ...
 ,market_date
 , julianday('now') - julianday (market_date) as datesbetweenmktdate -- number of days  btw now and each market day
 ,(julianday('now') - julianday(market_date)) / 365.25 as more_example
,(julianday('now') - julianday(market_date)) * 24 as even_more
